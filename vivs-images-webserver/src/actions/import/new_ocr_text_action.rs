...
impl AnalysisTaskItemProcessor<Arc<FilePathComparisonModel>, String, HashSet<String>, Arc<ImageOcrText>> for OcrTextProcessor {
    async fn get_analysis(&self, pool: Pool<Sqlite>, log_prog_listener: Option<LogProgListenerPair>) -> Result<Arc<FilePathComparisonModel>, Box<dyn std::error::Error + Send>> {
        get_image_path_comparison_ocr_text_table_analysis(&pool, log_prog_listener).await
            .map(|v| Arc::new(v))
    ...
    async fn process_task_item(&self, task_item: String, _pool: Pool<Sqlite>) -> Result<Arc<ImageOcrText>, Box<dyn std::error::Error + Send>> {
    ...
    async fn process_task_output(&self, task_output: Arc<ImageOcrText>, pool: SqlitePool) -> Result<(), Box<dyn std::error::Error + Send>> {
        execute_insert_image_ocr_text_sql(&task_output, pool).await
            .map_err(|e| Box::new(std::io::Error::other(format!("{}", e))) as Box<dyn std::error::Error + Send>)
    }

    async fn task_already_completed(&self, task_input: &String, pool: Pool<Sqlite>) -> Result<bool, Box<dyn std::error::Error + Send>> {
        query_ocr_text_table_count(&task_input, &pool).await
            .map(|v| v > 0)
            .map_err(|e| Box::new(std::io::Error::other(format!("{}", e))) as Box<dyn std::error::Error + Send>)
    }
