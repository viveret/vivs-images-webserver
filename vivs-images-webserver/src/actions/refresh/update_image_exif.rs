    ...
    async fn run_task(&self, pool: Pool<Sqlite>, send: TaskToWorkerSender, dry_run: bool, task_id: u32) -> actix_web::Result<(), Box<dyn std::error::Error + Send>> {
        ...
        for missing_from_db in analysis.files_missing_from_sql {
            ...
            let exif = match extract_image_exif(&missing_from_db, &options) {
                ...
                Err(e) => {
                    task_to_worker_send_helper2(&send, TaskToWorkerMessage::LogError(task_id, format!("extract image Exif error: {}", e)))?;
                    None
                },
            };
            ...
            if let Some(exif) = exif {
                ...
                match r {
                    Ok(()) => {
                        task_to_worker_send_helper2(&send, TaskToWorkerMessage::LogInfo(task_id, format!("Updated Exif to {} for {}", exif_str, missing_from_db)))?;
                    },
                    Err(e) => {
                        task_to_worker_send_helper2(&send, TaskToWorkerMessage::LogError(task_id, format!("update db image Exif error: {}", e)))?;
                    }
                }
            }
