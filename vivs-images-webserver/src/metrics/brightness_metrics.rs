...
pub async fn get_image_path_comparison_analysis(pool: &SqlitePool) -> actix_web::Result<Pin<Box<BrightnessMissingAnalysis>>> {
    ...
    log.push_str(&format!("Comparing {} images on disk to {} in brightness sql list", 
    ...
    Ok(Pin::new(Box::new(BrightnessMissingAnalysis {
        total_differences,
        files_missing_from_sql,
        files_missing_from_disk,
        message: format!("There are {} brightness file differences", total_differences),
        log, log_error
    })))
}
