...
pub async fn get_image_path_comparison_aspect_ratio_table_analysis(pool: &SqlitePool, log_prog_listener: Option<LogProgListenerPair>) -> actix_web::Result<FilePathComparisonModel> {
...
pub async fn get_aspect_ratio_missing_in_sql_count(pool: &SqlitePool) -> actix_web::Result<(usize, String)> {
...
pub async fn get_aspect_ratio_missing_on_disk_count(pool: &SqlitePool) -> actix_web::Result<(usize, String)> {
    let analysis = get_image_path_comparison_aspect_ratio_table_analysis(pool, None).await?;
